...
def plan_action_toeplitz(
    ...
    """
    Block-Toeplitz GEMM planner for discrete action spaces.

    Exploits exact orthogonality (ortho_a=True) to express the entire H-step
    trajectory as a single closed-form linear system Z = ZIR + W_toeplitz X.
    ZIR and W_toeplitz are read from agent.get_toeplitz_cache() — built once,
    reused until A changes. Each plan_iter is a single dense GEMM.

    Requires ortho_a=True (linear superposition only holds for A ∈ O(d)).
    """
    gamma  = cfg.algo.gamma if cfg else gamma
    n_acts = cfg.env.n_actions if cfg else agent.n_actions
    device = next(agent.parameters()).device
    ...
    with torch.no_grad():
        ...
        W_toeplitz, gammas, A_stack = agent.get_toeplitz_cache(horizon, gamma)
    ...
    for _ in range(plan_iters):
        ...
        X_flat = (probs @ B.T).reshape(horizon * d, 1)                  # [H*d, 1]

        Z    = ZIR + (W_toeplitz @ X_flat).reshape(horizon, d)          # [H, d]
...
def plan_action_continuous(
    ...
    for _ in range(plan_iters):
        ...
        for t in range(horizon):
            z_t = agent.dyn_step(z_t, (B @ u[t]).unsqueeze(0))
...
def plan_action_toeplitz_continuous_batch(
    ...
    W_toeplitz and A_stack are read from agent.get_toeplitz_cache() — built
    once per (horizon, gamma) key, shared across all N states and across all
    planning calls until the trainer invalidates the cache after opt.step().
    ZIR (state-dependent) is computed per call via a cheap einsum over the
    cached A_stack.
    ...
    for _ in range(plan_iters):
        ...
        if cumulative:
            v_vals = agent.v_net(Z.reshape(N * horizon, d)).reshape(N, horizon)
