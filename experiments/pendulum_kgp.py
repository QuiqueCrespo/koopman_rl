...
if __name__ == "__main__":
    ...
    env    = gym.make("Pendulum-v1")
    agent  = KoopmanGradientPlanner(state_dim=STATE_DIM, d=D, n_actions=ACTION_DIM)
    ...
    print("=" * 60)
    print("  KoopmanGradientPlanner — Pendulum-v1 Sanity Check")
    print(f"  state_dim={STATE_DIM}  d={D}  action_dim={ACTION_DIM}  scale={ACTION_SCALE}")
    print(f"  horizon={PLAN_HORIZON}  plan_iters={PLAN_ITERS}  N={N_STEPS} steps")
    print(f"  reward_scale={REWARD_SCALE}  viz → {VIZ_DIR}/  (every {VIZ_EVERY} steps)")
    print("=" * 60)
    print(f"\n[Warmup: {WARMUP} random transitions...]\n")
    ...
    ep_return = 0.0
    episode_returns  = []
    koop_log, v_log  = [], []       # per-1k averages for dashboard
    recent_koop, recent_v = [], []
    ...
    for step in range(1, N_STEPS + 1):
        ...
        if step < WARMUP:
            action = np.random.uniform(-ACTION_SCALE, ACTION_SCALE, size=(ACTION_DIM,))
        else:
            action = agent.act_plan_continuous(
                state, horizon=PLAN_HORIZON, plan_iters=PLAN_ITERS,
                action_scale=ACTION_SCALE,
            )
            action = action + np.random.normal(0, noise * ACTION_SCALE, size=action.shape)
        ...
        s_b   = torch.from_numpy(batch["states"]).to(device)
        ns_b  = torch.from_numpy(batch["next_s"]).to(device)
        a_b   = torch.from_numpy(batch["actions"]).to(device)
        r_b   = torch.from_numpy(batch["rewards"]).to(device)
        d_b   = torch.from_numpy(batch["dones"]).to(device)
        ...
        # Koopman: ||normalize(A z + B a) − z_{t+1}_tgt||²
        z_pred = F.normalize(z_src @ agent.A.T + a_b @ agent.B.T, dim=-1)
        L_koop = (z_pred - z_dst_tgt.detach()).pow(2).sum(dim=-1).mean()

        # Reconstruction anchor (anti-collapse)
        L_recon = (agent.decoder(z_src) - s_b).pow(2).mean()

        # 1-step TD with reward normalisation (keeps V in [-150, 0] range)
        with torch.no_grad():
            V_next = target.v_net(z_dst_tgt)
            y_td   = r_b / REWARD_SCALE + GAMMA * V_next * (1.0 - d_b)
        L_v = (agent.v_net(z_src) - y_td).pow(2).mean()

        loss = LAMBDA_KOOP * L_koop + LAMBDA_RECON * L_recon + LAMBDA_V * L_v
        opt.zero_grad()
        loss.backward()
        nn.utils.clip_grad_norm_(agent.parameters(), max_norm=10.0)
        opt.step()
        ...
        if step % 1_000 == 0:
            ...
            koop_log.append(mk)
            v_log.append(mv)
            recent_eps = episode_returns[-10:] if episode_returns else []
            print(
                f"  step {step:5d}  noise={noise:.3f}  "
                f"L_koop={mk:.4f}  L_v={mv:.4f}  "
                f"ret_mean={np.mean(recent_eps) if recent_eps else float('nan'):.1f}"
            )
            recent_koop.clear(); recent_v.clear()

        if step % VIZ_EVERY == 0:
            plot_dashboard(agent, episode_returns, koop_log, v_log, buf, step)
            plot_filmstrip(agent, step)

    # ── Evaluation ──────────────────────────────────────────────────────────

    print("\n--- Evaluation (10 episodes, plan_action_continuous) ---")
    eval_env    = gym.make("Pendulum-v1")
    eval_returns = []
    ...
    for ep in range(10):
        ...
        print(f"  ep {ep+1:2d}  return: {ret:.1f}")

    eval_env.close()
    mean_ret = np.mean(eval_returns)
    print(f"\nMean return: {mean_ret:.1f}  (target > -300)")
    print(f"{'PASS' if mean_ret > -300 else 'FAIL — check gradient/saturation'}")

    # ── Checkpoint ───────────────────────────────────────────────────────────

    save_checkpoint(agent, target, episode_returns, koop_log, v_log, mean_ret)

    # ── Final visualizations ─────────────────────────────────────────────────

    plot_dashboard(agent, episode_returns, koop_log, v_log, buf, N_STEPS)
